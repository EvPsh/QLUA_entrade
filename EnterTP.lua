...
function GetPosition(seccode)

	for i = 0, getNumberOf("FUTURES_CLIENT_HOLDING") - 1 do
	   -- ЕСЛИ строка по нужному инструменту И чистая позиция не равна нулю ТО
	   if getItem("FUTURES_CLIENT_HOLDING", i).sec_code == seccode and getItem("FUTURES_CLIENT_HOLDING", i).totalnet ~= 0 then
	      	return getItem("FUTURES_CLIENT_HOLDING",i).totalnet	-- Количество лотов 
