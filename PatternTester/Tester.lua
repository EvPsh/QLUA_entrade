...
Classcode =
"SPBFUT"                                               -- код класса инструмента/бумаги, если нужен фондовый рынок - вводить TQBR вместо SPBFUT. Пока не используется.
...
function OnInit()
    --инициализация
    scName =
    "PatternTester"          -- string.match(debug.getinfo(1).short_src, "\\([^\\]+)%.lua$") -- получение имени запущенного скрипта
...
function EndOfWord(n, txt)
    ...
    elseif (n >= 2 and n <= 4) then
        return txt .. en2
    elseif (n == 0 or n >= 5 and n <= 20) then
        return txt .. en3
    end

    if (n > 20 and n <= 100) then
        local z = n % 10
        if (z == 0 or z >= 5 and z <= 9) then
            return txt .. en3
        elseif (z == 1) then
            return txt
        elseif (z >= 2 and z <= 4) then
            return txt .. en2
        else
            return txt
        end
...
function getTimeFrame(m) -- получаем таймфрейм из разницы между свечами
    ...
    local datetime = os.date("!*t", sTime)
    local time1 = StrText(datetime.hour) ..
    ":" .. StrText(datetime.min) .. ":" .. StrText(datetime.sec)                                         -- возвращаем время в виде HH:MM:SS

    sTime = tostring(os.time(m.T[2]))
    datetime = os.date("!*t", sTime)
    local time2 = StrText(datetime.hour) ..
    ":" .. StrText(datetime.min) .. ":" .. StrText(datetime.sec)                                         -- возвращаем время в виде HH:MM:SS
...
function f_cb(t_id, msge, x, y)
    ...
    if (msge == QTABLE_LBUTTONDBLCLK) then
        ...
        if (x ~= 0) then                              -- в любом месте строки (не только в 1ой колонке)
            ...
            local lab_id = GetCell(t_id, x, 6)
            .image                                    -- 6 колонка - ширина колонки 0, идентификатор метки (не для вывода, для удаления с графика)
    ...
    if rows ~= 0 then
        for i = 1, rows do                                              -- номер строк с "1"
            for j = 0, col - 1 do                                       -- номер колонок с "0" до n - 1
                txt = txt ..
                tostring(GetCell(t_id, i, j).image) .. ";"              -- .image - это строковое значение, .value - это number
            end
            txt = txt .. "\n"
            CSV:write(txt)
...
function lightAllTable(t_id, sgl, x) -- раскраска строки цветом
    ...
    if signal < 0 then
        SetColor(t_id, x, QTABLE_NO_INDEX, RGB(255, 193, 193), QTABLE_DEFAULT_COLOR, RGB(255, 193, 193),
            QTABLE_DEFAULT_COLOR)
    elseif signal > 0 then
        SetColor(t_id, x, QTABLE_NO_INDEX, RGB(193, 255, 193), QTABLE_DEFAULT_COLOR, RGB(193, 255, 193),
            QTABLE_DEFAULT_COLOR)
    elseif signal == 0 then
        SetColor(t_id, x, QTABLE_NO_INDEX, RGB(220, 220, 220), QTABLE_DEFAULT_COLOR, RGB(220, 220, 220),
            QTABLE_DEFAULT_COLOR)
    end
end
